...
router.put("/", function (req, res) {
	...
	try {
		...
		const { json, deletedRows } = convertSheetToJsonArray(worksheet, cols);
...
const convertSheetToJsonArray = function (ws, colIndex) {
	importantCols = {
		key: colIndex.Key,
		name: colIndex.Name,
	};

	const { ws: newWs, deletedRows } = detectFalseApplications(
		ws,
		Object.values(importantCols)
	);
